function [data] = SEV2mat(SEV_DIR, varargin) %varargin - variable arguments in
...
for i = 1:length(file_list)
    ...
    if streamHeader.fileVersion > 0
        ...
        warning('%s has empty header; assuming ch %d format %s and fs = %.2f\nupgrade to OpenEx v2.18 or above\n', ...
            name, file_list(i).chan, streamHeader.dForm, streamHeader.fs);
    end
...
for ev = 1:numel(eventNames)
    ...
    data.(file_list_temp(1).varName).data = zeros(numel(channels), total_samples, dForm);
    ...
    for i = loop
        ...
        for j = hour_values
            ...
            if CHANNEL > 0
                data.(varname).data(1,this_offset:this_offset+file_list_temp(file_num).npts-1) = fread(fid, inf, ['*' dForm])';
            else
                data.(varname).data(ind,this_offset:this_offset+file_list_temp(file_num).npts-1) = fread(fid, inf, ['*' dForm])';
            end
